...
<template><div class="drill-grid"><el-card shadow="never"><el-tag>综合题 {{index+1}}/{{questions.length}}</el-tag><h3>{{q[0]}}</h3><el-space direction="vertical" fill style="width:100%"><el-button v-for="(option,i) in q[1]" :type="chosen===i?(i===q[2]?'success':'danger'):''" @click="answer(i)">{{option}}</el-button></el-space><el-alert v-if="chosen>=0" :title="q[3]" :type="chosen===q[2]?'success':'error'" :closable="false"/><el-button link @click="next">下一题 →</el-button></el-card><el-card class="timer-card" shadow="never"><small>{{phase==='prepare'?'准备 1 分钟':phase==='speak'?'回答 2 分钟':'完成'}}</small><strong>{{clock}}</strong><el-button type="primary" @click="start">开始</el-button><el-button @click="reset">重置</el-button></el-card></div></template>
